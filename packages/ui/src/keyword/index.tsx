  ...
  return (
    <div className="space-y-2">
      <div className="space-y-1">
        <h2 className="text-text">키워드</h2>
        <div className="border border-gray5 p-2 rounded-sm flex justify-between gap-2 items-center text-description min-h-keyword">
          <div className="flex gap-3">
            {keywords.map((keyword) => (
              <RectangleButton
                key={keyword}
                onClick={() => onDeleteKeyword(keyword)}
                className="border border-gray5 py-1 px-3 truncate max-w-20 sm:max-w-24"
              >
                {keyword}
              </RectangleButton>
            ))}
      ...
      <ul className="list-disc px-3.5 text-gray3 text-description">
